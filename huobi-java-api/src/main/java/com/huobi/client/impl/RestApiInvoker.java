      ...
      if (json.containKey("status")) {
        ...
        if ("error".equals(status)) {
          ...
          throw new HuobiApiException(
              HuobiApiException.EXEC_ERROR, "[Executing] " + err_code + ": " + err_msg);
        ...
        if (code != 200) {
          ...
          throw new HuobiApiException(
              HuobiApiException.EXEC_ERROR, "[Executing] " + code + ": " + message);
        }
      } else {
        throw new HuobiApiException(
    ...
    try {
      ...
      if (response.code() != 200) {
        throw new HuobiApiException(
            ...
            "[Invoking] Response Status Error : "
                + response.code()
                + " message:"
                + response.message());
      }
      ...
      call.enqueue(
          new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
              FailedAsyncResult<T> result =
                  new FailedAsyncResult<>(
            ...
            public void onResponse(Call call, Response response) {
              ...
              try {
                SucceededAsyncResult<T> result =
                    new SucceededAsyncResult<>(request.jsonParser.parseJson(jsonWrapper));
                callback.onResponse(result);
              } catch (Exception e) {
