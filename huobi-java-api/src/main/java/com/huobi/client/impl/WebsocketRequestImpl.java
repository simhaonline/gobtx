...
class WebsocketRequestImpl {
  ...
  WebsocketRequest<AccountEvent> subscribeAccountEvent(
    ...
    request.authHandler = (connection) -> connection.send(Channels.accountChannel(mode));
    request.jsonParser =
        (jsonWrapper) -> {
  ...
  WebsocketRequest<AccountListEvent> requestAccountListEvent(
    ...
    WebsocketRequest<AccountListEvent> request =
        new WebsocketRequest<AccountListEvent>(subscriptionListener, errorHandler);
    request.authHandler =
        (connection) -> {
  ...
  WebsocketRequest<OrderListEvent> requestOrderListEvent(
      ...
      SubscriptionErrorHandler errorHandler) {
    WebsocketRequest<OrderListEvent> request =
        new WebsocketRequest<>(subscriptionListener, errorHandler);
    ...
    request.authHandler =
        (connection) -> {
          ...
          String startDateString =
              ordersRequest.getStartDate() == null
                  ? null
                  : DateFormatUtils.format(ordersRequest.getStartDate(), "yyyy-MM-dd");
          String endDateString =
              ordersRequest.getEndDate() == null
                  ? null
                  : DateFormatUtils.format(ordersRequest.getEndDate(), "yyyy-MM-dd");
          ...
          requestTopic.put(
              "from", ordersRequest.getStartId() != null ? ordersRequest.getStartId() + "" : null);
          requestTopic.put(
              "direct",
              ordersRequest.getDirect() != null ? ordersRequest.getDirect().toString() : null);
          requestTopic.put(
              "size", ordersRequest.getSize() != null ? ordersRequest.getSize() + "" : null);
