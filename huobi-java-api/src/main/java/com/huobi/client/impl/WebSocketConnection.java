...
public class WebSocketConnection extends WebSocketListener {
  ...
  WebSocketConnection(
    ...
    try {
      ...
      log.error("fail subscribe init {}", e);
    }

    this.okhttpRequest =
        request.authHandler == null
  ...
  public void onMessage(WebSocket webSocket, ByteString bytes) {
    ...
    try {
      if (request == null) {
        log.error("[Sub][" + this.connectionId + "] request is null");
        closeOnError();
        return;
      }
      ...
      try {
        ...
        log.error("[Sub][" + this.connectionId + "] Receive message error: " + e.getMessage());
        closeOnError();
        return;
      }
  ...
  private void onError(String errorMessage, Throwable e) {
    if (request.errorHandler != null) {
      HuobiApiException exception =
          new HuobiApiException(HuobiApiException.SUBSCRIPTION_ERROR, errorMessage, e);
  ...
  public void onOpen(WebSocket webSocket, Response response) {
    ...
    if (request.authHandler != null) {
      ...
      builder
          .putToUrl(ApiSignature.op, ApiSignature.opValue)
