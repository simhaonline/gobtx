...
class RestApiRequestImpl {
  ...
  private Request createRequest(String url, String address, UrlParamsBuilder builder) {
    ...
    if (builder != null) {
      if (builder.hasPostParam()) {
        ...
        return new Request.Builder()
            .url(requestUrl + builder.buildUrl())
  ...
  private Request createRequestWithSignature(
      String url, String address, String host, UrlParamsBuilder builder) {
    if (builder == null) {
  ...
  RestApiRequest<Long> getExchangeTimestamp() {
    ...
    request.jsonParser = (json -> TimeService.convertCSTInMillisecondToUTC(json.getLong("data")));
    return request;
  }
  ...
  RestApiRequest<PriceDepth> getPriceDepth(String symbol, Integer size) {
    ...
    UrlParamsBuilder builder =
        UrlParamsBuilder.build().putToUrl("symbol", symbol).putToUrl("type", "step0");
              ...
              (item) -> {
                ...
                dataArrayIn.forEach(
                    (itemIn) -> {
                      ...
                      trade.setTimestamp(
                          TimeService.convertCSTInMillisecondToUTC(itemIn.getLong("ts")));
  ...
  RestApiRequest<List<Currency>> getCurrencyInfo(String currency, Boolean authorizedUser) {
    ...
    UrlParamsBuilder builder =
        UrlParamsBuilder.build()
            ...
            .putToUrl(
                "authorizedUser", authorizedUser == null ? "true" : authorizedUser.toString());
  ...
  RestApiRequest<List<Withdraw>> getWithdrawHistory(
      String currency, Long fromId, Integer size, QueryDirection queryDirection) {
    InputChecker.checker()
        .checkCurrency(currency)
        .shouldNotNull(fromId, "fromId")
  ...
  RestApiRequest<Long> repayLoan(long loadId, BigDecimal amount) {
    ...
    RestApiRequest<Long> request = new RestApiRequest<>();
    UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("amount", amount);
  ...
  RestApiRequest<Long> repayCrossMarginLoan(CrossMarginRepayLoanRequest repayLoanRequest) {
    ...
    RestApiRequest<Long> request = new RestApiRequest<>();
    UrlParamsBuilder builder =
        UrlParamsBuilder.build().putToPost("amount", repayLoanRequest.getAmount());
  ...
  RestApiRequest<List<CrossMarginLoanOrder>> getCrossMarginLoanHistory(
      CrossMarginLoanOrderRequest loanOrderRequest) {
  ...
  RestApiRequest<Long> createOrder(NewOrderRequest newOrderRequest) {
    InputChecker.checker()
        .checkSymbol(newOrderRequest.getSymbol())
    ...
    if (newOrderRequest.getType() == OrderType.SELL_LIMIT
      ...
      InputChecker.checker().shouldNotNull(newOrderRequest.getPrice(), "Price");
    }
    if (newOrderRequest.getType() == OrderType.SELL_MARKET
        || newOrderRequest.getType() == OrderType.BUY_MARKET) {
      InputChecker.checker().shouldNull(newOrderRequest.getPrice(), "Price");
    }
    Account account =
        AccountsInfoMap.getUser(apiKey)
            .getAccount(newOrderRequest.getAccountType(), newOrderRequest.getSymbol());
    if (account == null) {
    ...
    String stopOp =
        newOrderRequest.getOperator() != null ? newOrderRequest.getOperator().getOperator() : "";
    UrlParamsBuilder builder =
        UrlParamsBuilder.build()
  ...
  RestApiRequest<List<Order>> getOpenOrders(OpenOrderRequest openOrderRequest) {
    ...
    Account account =
        AccountsInfoMap.getUser(apiKey).getAccount(accountType, openOrderRequest.getSymbol());
    if (account == null) {
      throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] No such account");
    }
        ...
        (jsonWrapper -> {
          ...
          dataArray.forEach(
              (item) -> {
                ...
                order.setOperator(
                    StopOrderOperator.find(item.getStringOrDefault("operator", null)));
                orderList.add(order);
              });
          return orderList;
        });
  ...
  RestApiRequest<BatchCancelResult> cancelOpenOrders(
    ...
    Account account =
        AccountsInfoMap.getUser(apiKey)
            .getAccount(
                cancelOpenOrderRequest.getAccountType(), cancelOpenOrderRequest.getSymbol());
    if (account == null) {
      throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] No such account");
    }
    ...
    request.request =
        createRequestByPostWithSignature("/v1/order/orders/batchCancelOpenOrders", builder);
    request.jsonParser =
        (jsonWrapper -> {
  ...
  RestApiRequest<List<MatchResult>> getMatchResults(MatchResultRequest matchResultRequest) {
    InputChecker.checker()
        .checkSymbol(matchResultRequest.getSymbol())
  ...
  RestApiRequest<List<DepositAddress>> getDepositAddress(String currency) {
    InputChecker.checker().checkCurrency(currency);
    RestApiRequest<List<DepositAddress>> request = new RestApiRequest<>();
    UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("currency", currency);
  ...
  RestApiRequest<List<Order>> getOrders(OrdersRequest req) {
    InputChecker.checker().checkSymbol(req.getSymbol()).shouldNotNull(req.getStates(), "states");
    RestApiRequest<List<Order>> request = new RestApiRequest<>();
          ...
          dataArray.forEach(
              (item) -> {
                ...
                order.setCanceledTimestamp(
                    TimeService.convertCSTInMillisecondToUTC(
                        item.getLongOrDefault("canceled-at", 0)));
                order.setFinishedTimestamp(
                    TimeService.convertCSTInMillisecondToUTC(
                        item.getLongOrDefault("finished-at", 0)));
                order.setOrderId(item.getLong("id"));
                order.setSymbol(item.getString("symbol"));
                order.setPrice(item.getBigDecimal("price"));
                order.setCreatedTimestamp(
  ...
  RestApiRequest<List<FeeRate>> getFeeRate(String symbol) {
    ...
    UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbols", symbol);
  ...
  RestApiRequest<EtfSwapConfig> getEtfSwapConfig(String etfSymbol) {
    ...
    UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("etf_name", etfSymbol);
  ...
  RestApiRequest<Void> etfSwap(String etfSymbol, int amount, EtfSwapType swapType) {
    ...
    RestApiRequest<Void> request = new RestApiRequest<>();
    UrlParamsBuilder builder =
        UrlParamsBuilder.build().putToPost("etf_name", etfSymbol).putToPost("amount", amount);
  ...
  public RestApiJsonParser<Order> getOrderParser() {
    return (jsonWrapper -> {
      ...
      order.setAccountType(
          AccountsInfoMap.getAccount(apiKey, data.getLong("account-id")).getType());
