...
public class SubscribeOrderUpdate {
  ...
  public static void main(String[] args) {
    ...
    client.subscribeOrderUpdateEvent(
        symbol,
        (event -> {
    ...
    client.subscribeOrderUpdateNewEvent(
        symbol,
        (event -> {
          System.out.println(event.getData().toString());
        }));
    ...
    OrdersRequest ordersRequest =
        new OrdersRequest(
            symbol, stateList, typeList, startDate, today, startOrderId, 20, QueryDirection.PREV);

    client.requestOrderListEvent(
        ordersRequest,
        orderListEvent -> {
    ...
    client.requestOrderDetailEvent(
        startOrderId,
        orderListEvent -> {
