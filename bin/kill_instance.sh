...
double_kill(){
    EXIST_ONE=`ps -aef | grep "\-Dinstance_name=${INS_CNT}"  | grep "\-Dapp.instance=${INSTANCE}" | wc -l`
    echo "Try do Kill through the ps grep $EXIST_ONE"
    ROUND=0
    while [[ "$EXIST_ONE" = "1" && $ROUND -lt 7 ]]; do
        echo "-Dinstance_name=${INS_CNT} -Dapp.instance=${INSTANCE} Still Live, PID gone, so Grep to Kill"
        for pid in $(ps -aef | grep "\-Dinstance_name=${INS_CNT}"  | grep "\-Dapp.instance=${INSTANCE}" | awk '{print $2}'); do kill $pid; done
        sleep 10
        EXIST_ONE=`ps -aef | grep "\-Dinstance_name=${INS_CNT}"  | grep "\-Dapp.instance=${INSTANCE}" | wc -l`
        ROUND=$((ROUND+1))
...
detect(){
    ...
    if [ "$EXIST_ONE" = "1" ]; then
        echo "STILL exist force kill -9 \-Dinstance_name=${INS_CNT} \-Dapp.instance=${INSTANCE} "
        for pid in $(ps -aef | grep "\-Dinstance_name=${INS_CNT}"  | grep "\-Dapp.instance=${INSTANCE}" | awk '{print $2}'); do kill -9 $pid; done
        return 1
    else
        return 0
    fi
